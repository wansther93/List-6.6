...
export const ScheduleDetailModal: React.FC<ScheduleDetailModalProps> = ({
  ...
  useEffect(() => {
    ...
    const userStatus = existingUserAnime?.status || null;

    // Sincroniza metadados oficiais automaticamente da API (AniList / Jikan) para precisão absoluta de status e episódios
    fetchFreshAnimeDetails(anime.title, malId, userStatus)
      .then((data) => {
  ...
  const effectiveAggregatedStatus: AnimeAggregatedStatus =
    ...
    resolveAnimeAggregatedStatus({
      ...
      userTrackerStatus: existingUserAnime?.status || null,
